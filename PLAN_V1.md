...
```mermaid
flowchart LR
    N["Navegador o Postman"] -->|"HTTP puerto 8025"| A["API investigacion<br/>Python FastAPI + SQLAlchemy"]
    A -->|"SQL parametrizado"| B[("PostgreSQL puerto 15440<br/>base investigacion_local")]
```
...
| `ProyectosDeAula/db_scripts/postgresql/investigacion.pg.sql` | El **DDL**: 223 líneas, 19 `CREATE TABLE`, 19 claves foráneas |
...
```yaml
  api-investigacion:
    environment:
      ConnectionStrings__PostgreSQL: "Server=postgres,1433;Database=investigacion_local;User Id=sa;Password=Aplicacionweb123!;TrustServerCertificate=True;"
```
...
flowchart LR
    ...
    CMP --> API["api-investigacion<br/>ConnectionStrings__PostgreSQL<br/>sobreescribe las variables del compose"]
...
```yaml
    environment:
      ...
      ConnectionStrings__PostgreSQL: "Server=postgres,1433;Database=investigacion_local;User Id=sa;Password=${MSSQL_SA_PASSWORD};TrustServerCertificate=True;"
```
...
| `api_investigacion\Repositorios` | La interfaz y el SQL con SQLAlchemy | **Capa 3 — datos**: no conoce HTTP |
...
New-Item -ItemType File db\init.sql, db\init.sh

# la API (paso 5)
New-Item -ItemType File `
  api_investigacion\ApiInvestigacion/requirements.txt, api_investigacion\Dockerfile,
  api_investigacion\main.py, api_investigacion\las variables del compose,
  api_investigacion\Modelos\AreaConocimiento.py,
  api_investigacion\Peticiones\AreaConocimientoCrear.py,
  api_investigacion\Peticiones\AreaConocimientoReemplazo.py,
  api_investigacion\Peticiones\AreaConocimientoActualizar.py,
  api_investigacion\Repositorios\IRepositorioAreaConocimiento.py,
  api_investigacion\Repositorios\RepositorioAreaConocimientoPostgreSQL.py,
  api_investigacion\Servicios\IServicioAreaConocimiento.py,
  api_investigacion\Servicios\ServicioAreaConocimiento.py,
  api_investigacion\Controllers\AreaConocimientoController.py,
  api_investigacion\Excepciones\NoEncontradoExcepcion.py,
  api_investigacion\pruebas\PruebaCapas/requirements.txt,
  api_investigacion\pruebas\Programa.py
...
### Paso 5 — La API
Python / FastAPI + SQLAlchemy, capas con interfaces, para `area_conocimiento`: el modelo, las tres peticiones (crear / reemplazo / actualizar), la interfaz y el repositorio, la interfaz y el servicio, y el controlador con los siete endpoints. Más `main.py` con el ensamblador, `Dockerfile` y el proyecto `pruebas/` con un repositorio falso en memoria. **Unos 12 archivos**, no 45: esa es la ganancia de haber escogido una sola tabla.
```mermaid
flowchart TB
    ...
    R["RepositorioAreaConocimientoPostgreSQL<br/>SQL con SQLAlchemy"] -.->|implementa| IR
...
```powershell
python -m compileall api_investigacion
uvicorn --project api_investigacion\pruebas
```
...
curl http://localhost:8025/                                  # diagnostico: version v1
curl http://localhost:8025/api/area_conocimiento             # total: 218
curl "http://localhost:8025/api/area_conocimiento?limite=3"  # exactamente 3
curl http://localhost:8025/api/area_conocimiento/1A01        # una fila
curl -i http://localhost:8025/api/area_conocimiento/9Z99     # 404 con mensaje claro
```
...
8. Trabajo en Windows con VS Code (terminal integrada de PowerShell) y
   ...
   puerto 8025 y PostgreSQL el 15440.
...
5. Todo en español, Python sobre FastAPI con SQLAlchemy y el SQL a
   la vista. API en el puerto 8025, PostgreSQL en el 15440.
...
> aquí llevan el prefijo `paradigmas-investigacion-`, y la regla quedó en las convenciones
> de la constitución.
