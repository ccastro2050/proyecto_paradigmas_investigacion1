...
tiene 2: `./api_investigacion`, `./front_flask`.
...
| `EXPOSE 8025` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
- En la v1: `bd_v1` es un contenedor creado desde la imagen `postgres:16-alpine`,
  con el puerto interno 5432 **publicado** en el 15440 de su PC (`-p 15440:5432`).
...
flowchart LR
    ...
    CLIENTE["Cliente de base de datos<br/>(DBeaver, SQLTools, psql)"]
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTFLASK["SERVIDOR WEB (front)<br/>contenedor front-flask<br/>hostname: front-flask · escucha en 8078"]
            APIINVESTIGACION["SERVIDOR DE APLICACIONES<br/>contenedor api-investigacion<br/>hostname: api-investigacion · escucha en 8025"]
            POSTGRES[("SERVIDOR DE BASE DE DATOS<br/>PostgreSQL · contenedor postgres<br/>hostname: postgres · escucha en 5432")]
        end
    end
    NAV -->|"localhost:8025"| APIINVESTIGACION
    NAV -->|"localhost:8078"| FRONTFLASK
    APIINVESTIGACION -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    FRONTFLASK -->|"http://api-investigacion:8025<br/>al controlador, por el NOMBRE"| APIINVESTIGACION
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15440"| POSTGRES
```
...
services:                          # el mapa de TODOS los contenedores del sistema
  ...
  postgres:                        # ← este nombre es también su HOSTNAME interno
    ...
    ports:
      - "15440:5432"               # "puerto en su PC : puerto interno del contenedor"
    healthcheck:                   # cómo saber si la BD ya RESPONDE (no solo "existe")
      test: ["CMD-SHELL", "pg_isready -U paradigmas -d investigacion_local"]
      interval: 5s
      timeout: 5s
      retries: 10
  ...
  api-registros:
    ...
    volumes:
      - ./api_investigacion:/app        # el código montado: guardar un .py recarga la API sola
    ...
    ports:
      - "8025:8025"                # http://localhost:8025/docs
    environment:
      ...
      DB_POSTGRES: postgresql+asyncpg://paradigmas:paradigmas123@postgres:5432/investigacion_local
    depends_on:
      postgres:
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8025`, `localhost:15440`); entre contenedores, nombres de
...
docker network create proyecto_paradigmas_investigacion1_default

# 2. postgres
docker run -d --name paradigmas-investigacion-postgres --network proyecto_paradigmas_investigacion1_default --restart unless-stopped `
  -e "POSTGRES_USER=investigacion" `
  -e "POSTGRES_PASSWORD=Investigacion123!" `
  -e "POSTGRES_DB=investigacion_local" `
  -v pgdata:/var/lib/postgresql/data `
  -v "${PWD}/db/init.sql:/docker-entrypoint-initdb.d/init.sql:ro" `
  -p 15440:5432 postgres:16
...
docker run -d --name paradigmas-investigacion-api --network proyecto_paradigmas_investigacion1_default --restart unless-stopped `
  -e "DB_POSTGRES=postgresql+asyncpg://investigacion:Investigacion123!@postgres:5432/investigacion_local" `
  -v "${PWD}/api_investigacion:/app" `
  -p 8025:8025 api-investigacion uvicorn main:app --host 0.0.0.0 --port 8025 --reload

# 5. Construir la imagen de front-flask y encenderla
docker build -t front-flask ./front_flask
docker run -d --name paradigmas-investigacion-front --network proyecto_paradigmas_investigacion1_default --restart unless-stopped `
  -e "URL_API_INVESTIGACION=http://api-investigacion:8025" `
  -v "${PWD}/front_flask:/app" `
  -p 8078:8078 front-flask
...
```yaml
URL_API: http://api-investigacion:8025
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8025` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-investigacion:8025` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8025:8025`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8025`, el front
...
```powershell
docker network inspect proyecto_paradigmas_investigacion1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_paradigmas_investigacion1` produce
la red `proyecto_paradigmas_investigacion1_default`. Tres formas de confirmarlo:
...
docker inspect paradigmas-investigacion-front --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_paradigmas_investigacion1_default has active endpoints (name:"paradigmas-investigacion-front" …)
```
