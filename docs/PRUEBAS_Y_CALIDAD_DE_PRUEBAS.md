...
`api_investigacion/pruebas/prueba_capas.py`:
...
```python
# Con pytest, un framework de pruebas:
...
servicio depende de el protocolo del repositorio, no del motor**. Es el
...
| **Prueba de capas** | `api_investigacion/pruebas/` | Que las reglas funcionen **y** que las capas estén cortadas |
...
| Qué medir | Herramienta (Python) | Cómo se ve |
|---|---|---|
| Cobertura | **coverage.py** (`coverage run -m pytest` + `coverage report`) | % por línea y por rama |
| Complejidad / CRAP | **radon** (`radon cc`) | complejidad ciclomática por función |
| Mutación | **mutmut** (`mutmut run`) | mutantes creados / matados / sobrevivientes |
...
10. En este repositorio: los criterios de `2_spec.md`, el smoke de
    `7_quickstart.md` y la prueba de capas de `api_investigacion/pruebas/` — la
