...
Cuando su API atiende `GET /api/area_conocimiento`, ¿en qué gasta el tiempo?
...
@router.get("/api/area_conocimiento")
...
async def obtener_todos(self, limite: int) -> list[dict]:
    async with self._engine.connect() as conexion:
        resultado = await conexion.execute(
            text("SELECT * FROM area_conocimiento LIMIT :limite"), {"limite": limite})
        ...
```
