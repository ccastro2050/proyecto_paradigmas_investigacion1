...
def test_crear_area_conocimiento():
    creado = servicio.crear_area_conocimiento(datos)
    # (sin assert: si crear_area_conocimiento guarda mal, esta prueba PASA)
```

La que sí protege lleva **una línea que puede fallar**:

```python
def test_crear_area_conocimiento_guarda_el_nombre():
    creado = servicio.crear_area_conocimiento(datos)
