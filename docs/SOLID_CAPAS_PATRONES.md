...
│ controllers/area_conocimiento_controller.py                  │
...
│ servicios/servicio_area_conocimiento.py                      │
...
└────────────────┬────────────────────────────────────┘
                 ...
                 │     — a través de la INTERFAZ IRepositorioAreaConocimiento
                 ▼
┌─────────────────────────────────────────────────────┐
│ CAPA 3 — REPOSITORIO (datos)                        │
│ repositorios/repositorio_area_conocimiento_postgresql.py     │
│ El SQL: traduce filas ↔ objetos. NO conoce HTTP.    │
│ NO decide negocio.                                  │
└────────────────┬────────────────────────────────────┘
                 │  ④ SELECT … FROM area_conocimiento WHERE codigo = 'PR001'
                 ▼
          ┌───────────────┐
          │ BASE DE DATOS │  PostgreSQL — investigacion_local
          └───────┬───────┘
                  │
...
|---|---|---|---|
| **Controller** | HTTP: rutas, códigos de estado, JSON | SQL y reglas de negocio | `controllers/area_conocimiento_controller.py` |
| **Servicio** | Las reglas del negocio (¿existe? ¿se puede?) | Saber de HTTP o del motor de BD | `servicios/servicio_area_conocimiento.py` |
| **Repositorio** | El SQL y el mapeo fila ↔ objeto | Saber de HTTP o decidir negocio | `repositorios/repositorio_area_conocimiento_postgresql.py` |
...
**El mismo viaje cuando algo sale mal** — `GET /api/area_conocimiento/PR999`:

1. El **repositorio** no encuentra la fila y devuelve `None` — un HECHO,
   sin opinión.
2. El **servicio** decide qué significa ese hecho: "ese area_conocimiento no
...
Pregunta legítima: la carpeta de modelos (`models/area_conocimiento.py (Pydantic)`) existe en el
...
- El **modelo es LO QUE VIAJA entre estaciones**: el repositorio arma un
  `AreaConocimiento` desde la fila, el servicio lo razona, el controller lo
...
flowchart LR
    ...
    M["MODELO AreaConocimiento<br/>el paquete que viaja"]
    C -.->|"lo conoce"| M
    S -.->|"lo conoce"| M
    R -.->|"lo conoce"| M
...
`gran_area` no lo atraparía nadie hasta producción. Con modelo, lo atrapa el
...
@router.get("/api/area_conocimiento/{codigo}")
...
(`RepositorioAreaConocimientoMysqlMariaDB`) y una línea en la fábrica — controllers y
...
class RepositorioAreaConocimientoMariaDB:
    """Los mismos 5 métodos que promete IRepositorioAreaConocimiento."""

# ...y el ensamblador (ÚNICO archivo tocado) elegirá el motor:
repositorio = (RepositorioAreaConocimientoMariaDB(cadena)
               if motor == "mariadb"
               else RepositorioAreaConocimientoPostgreSQL(cadena))
```

### L — Sustitución de Liskov (*Liskov Substitution*)
> Donde sirve el tipo base, debe servir CUALQUIER implementación, sin sorpresas.

**Su momento son la v3 y la v4:** los tres repositorios de area_conocimiento
...
servicio = ServicioAreaConocimiento(RepositorioFalso())   # ← el servicio NI SE ENTERA
```
...
**En la v1:** `IRepositorioAreaConocimiento` tiene exactamente los 5 métodos del CRUD
de area_conocimiento — no un `IRepositorioUniversal` con 40 métodos. Cuando la v2
agregue persona, tendrá SU interfaz.

```python
# ✅ La interfaz de la v1: SOLO los 5 métodos del CRUD de area_conocimiento
...
**En la v1:** `ServicioAreaConocimiento` recibe **la interfaz** por constructor; solo
...
class ServicioAreaConocimiento:
    def __init__(self, repositorio: IRepositorioAreaConocimiento):   # ← interfaz, no clase
...
def crear_servicio_area_conocimiento() -> IServicioAreaConocimiento:
    repositorio = RepositorioAreaConocimientoPostgreSQL(os.environ["DB_POSTGRES"])
...
| **Inyección de dependencias** | creacional (IoC) | los constructores + el ensamblador (`crear_servicio_area_conocimiento()`) |
...
# ServicioAreaConocimiento lo usa SIN saber si detrás hay PostgreSQL o un
...
```python
class ServicioAreaConocimiento:
    def __init__(self, repositorio: IRepositorioAreaConocimiento):  # ← llega armado
        self._repositorio = repositorio

# El ÚNICO lugar que conoce la clase concreta es el ensamblador:
def crear_servicio_area_conocimiento() -> IServicioAreaConocimiento:
    return ServicioAreaConocimiento(RepositorioAreaConocimientoPostgreSQL(cadena))
```

### DTO por verbo — el body aterriza en un modelo que solo valida forma

```python
class AreaConocimiento(BaseModel):                       # POST: todo obligatorio
    codigo: str = Field(min_length=1, max_length=20)
    stock: int = Field(ge=0)
    # Un body inválido muere en 422 ANTES de tocar servicio o BD.

class AreaConocimientoActualizar(BaseModel):             # PATCH: todo opcional
    ...
```
...
def crear_repositorio_area_conocimiento(motor: str) -> IRepositorioAreaConocimiento:
    if motor == "mariadb":
        return RepositorioAreaConocimientoMariaDB(cadena_mariadb)
    return RepositorioAreaConocimientoPostgreSQL(cadena_postgres)
...
(`RepositorioAreaConocimientoPostgreSQL`, el `RepositorioFalso` de la prueba — y
...
7. En este repositorio: el [plan de la v1](spec_kit/versiones/v1_area_conocimiento/3_plan.md)
