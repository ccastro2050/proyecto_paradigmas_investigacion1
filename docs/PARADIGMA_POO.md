...
`RepositorioAreaConocimientoPostgreSQL` encapsula el engine de conexión y el SQL: nadie
...
es **composición sobre herencia** — y este proyecto la sigue: `ServicioAreaConocimiento`
...
`IRepositorioAreaConocimiento` puede ocupar el lugar de otra — el PostgreSQL real, el
...
classDiagram
    ...
    RepositorioAreaConocimientoPostgreSQL ..|> IRepositorioAreaConocimiento : POLIMORFISMO
    ...
    note for RepositorioAreaConocimientoPostgreSQL "ENCAPSULAMIENTO: la conexión y el SQL no salen de aquí"
```
...
modelo Pydantic: `class AreaConocimiento(BaseModel)` — AreaConocimiento ES-UN BaseModel y
...
classDiagram
    ...
    ServicioMal --|> RepositorioAreaConocimientoPostgreSQL : hereda del CONCRETO — quedó casado con PostgreSQL
...
sequenceDiagram
    ...
    participant R as IRepositorioAreaConocimiento (Protocol)
...
AreaConocimiento(codigo="PR009", nombre="Webcam", stock=5, valorunitario=120000)  # ✔ objeto válido
AreaConocimiento(codigo="PR009", nombre="Webcam", stock=-5, valorunitario=1)      # ✖ ValidationError
```

Mírelo con lentes de paradigma — Pydantic es dos paradigmas cooperando:

- Es **P.O.O.**: `AreaConocimiento` es una clase; cada petición válida se convierte en
...
1. **Frontera de entrada:** FastAPI recibe el body JSON, intenta construir el
   ...
   a la BD.
2. **Un modelo por semántica HTTP:** `AreaConocimiento` (POST: todo obligatorio),
   `AreaConocimientoReemplazo` (PUT: reemplazo completo) y `AreaConocimientoActualizar`
...
```
AreaConocimiento (Pydantic)          ← objeto de DATOS con validación (abstracción del dominio)
ServicioAreaConocimiento             ← objeto de NEGOCIO; compone un IRepositorioAreaConocimiento
IRepositorioAreaConocimiento         ← contrato (Protocol): abstracción pura
RepositorioAreaConocimientoPostgreSQL ← implementación concreta (encapsula SQL y conexión)
...
(`RepositorioAreaConocimientoMysqlMariaDB`) entrará por la misma puerta.
