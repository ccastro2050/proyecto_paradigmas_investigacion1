# Constitución del proyecto — Módulo Investigación
...
> constitución gana.
>
> Este proyecto es el **ejemplo de referencia** del módulo Investigación
...
`sys.path.append("../api_investigacion")` para importar sus modelos y sus servicios.
Funcionaría.
...
- **No se usa un ORM.** El SQL se escribe a mano, queda a la vista y
  **siempre** va parametrizado con `:parametro` (jamás concatenando
  ...
  de puente asíncrono al motor, pero **no genera SQL por nosotros**. Si una
  consulta está mal, está mal en un archivo que se puede leer.
- Dependencias: `fastapi`, `uvicorn`, `sqlalchemy[asyncio]`, `asyncpg` (el
  cliente del motor), `greenlet` y `pydantic`. Nada más.
- La documentación interactiva la genera FastAPI sola en `/docs`: **no se
  instala nada** para tenerla, y por eso el Artículo 9 puede exigir que los
  contratos estén a la vista.

> **Por qué sin ORM.** Un ORM escribiría el SQL por nosotros, y el punto del
> curso es justamente **ver ese SQL**. Con `text()` la consulta está escrita
> en el repositorio, se lee, se copia a un cliente de base de datos y se
> ejecuta igual. Lo que se pierde —migraciones automáticas, mapeo de
> relaciones— no hace falta aquí: la base viene dada (Artículo 5).
...
fuera** —con SSMS o SQLTools contra `localhost:15440`— y entonces se lee
...
| Cosa | Convención |
|---|---|
| Puertos del proyecto | front **8078** · API **8025** · PostgreSQL **15440** |
| Nombres de contenedor | Llevan el prefijo `paradigmas-investigacion-`: los nombres, como los puertos, **no se repiten** entre proyectos del semestre |
...
| | |
|---|---|
| Los valores del manual viven en | `front_flask/static/marca.css` |
