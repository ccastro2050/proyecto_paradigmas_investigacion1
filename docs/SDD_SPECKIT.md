...
> Aplicada aquí al **módulo Investigación**, con el stack de paradigmas:
> **Python, FastAPI y PostgreSQL**. El método es el mismo que en los
> demás módulos del curso; lo que cambia es el lenguaje y el motor.

> Aplicada aquí al **módulo Investigación**. El método es el mismo en
...
este curso usted lo vive: la [GUIA_IA.md de la versión](spec_kit/versiones/v1_area_conocimiento/GUIA_IA1.md) construye la versión
...
- **La versión en curso:**
  [spec_kit/versiones/v1_area_conocimiento/](spec_kit/versiones/v1_area_conocimiento/2_spec.md)
...
`PATCH /api/area_conocimiento/{codigo}` con body de la petición AreaConocimientoActualizar:
...
```markdown
### RF5 — Actualizar parcialmente (PATCH + body parcial)
PATCH /api/area_conocimiento/{codigo} con campos opcionales: solo se
...
| `ApiFacturas.pyproj` | ★ un paquete nuevo, si la versión lo exige |
```
...
| Tabla | PK | Semilla |
|---|---|---|
| area_conocimiento | id | 218 filas del Excel del módulo |
...
```bash
curl http://localhost:8025/api/area_conocimiento        # total: 8
curl -i http://localhost:8025/api/area_conocimiento/PR999   # → 404
```
...
2. **Medible o no es criterio.** Un criterio dice un número, un código de
   ...
   200 con las 218 filas semilla" sí.
...
```markdown
### RF6 — Retirar el área de conocimiento
DELETE /api/area_conocimiento/{id} retira el área de conocimiento del uso.
...
| "Debe ser rápido" | "El listado responde en menos de 1 s con las 218 filas semilla" |
...
|---|---|---|---|---|---|
| RF1 — listar | C1, C2 | §2 `GET /api/area_conocimiento` | Fase 5 · controlador | §2 comando 2 | 1 y 2 |
...
1. La carpeta de la versión es `vN_tema`, en minúsculas y con guion bajo
   (`v1_area_conocimiento`). El tema dice qué agrega la versión.
...
`main.py` ni en el `.pyproj`, y no cambia en nada cómo corre la API. Es
...
specify init mi_v1_area_conocimiento --integration copilot
specify version
```
...
```text
La spec dice:
    RF6 — Eliminar area_conocimiento
    DELETE /api/area_conocimiento/{codigo}. Devuelve filasEliminadas;
    inexistente -> 404.

clarify pregunta:
    El borrado de area_conocimiento, ¿es físico o lógico?
...
clarify NO crea un archivo: escribe dentro de 2_spec.md
    ## Clarificaciones
    C5 — DELETE, ¿físico o lógico? -> LÓGICO: la tabla area_conocimiento gana
...
[la sección de Clarificaciones de 2_spec.md](spec_kit/versiones/v1_area_conocimiento/2_spec.md),
...
Convergencia — v1
    ...
    Criterio 2  OK    GET /api/area_conocimiento devuelve las 8 filas semilla
...
[8_tasks.md](spec_kit/versiones/v1_area_conocimiento/8_tasks.md) —el
cierre— corriendo el smoke test del
[7_quickstart.md](spec_kit/versiones/v1_area_conocimiento/7_quickstart.md).
...
4. **Es evaluable y comparable.** Los `.md` se leen y se califican, y los
   cursos comparten la misma estructura sobre la misma `investigacion_local` con
   stacks distintos.
...
4. En este repositorio: el [spec kit completo](spec_kit/1_constitution.md)
   y la [GUIA_IA.md de la versión](spec_kit/versiones/v1_area_conocimiento/GUIA_IA1.md) que lo pone a prueba.
